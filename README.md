...
Word count is the default metric. It can be changed globally in the plugin settings or per block from the footer dropdown.
...
In Live Preview and Source mode, choose a metric from the footer to update the block's `metric=` option. Reading mode keeps the metric name static.
